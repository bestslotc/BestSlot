  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await sendVerificationEmail({
      ...
      fetchOptions: {
        ...
        onError: (ctx) => {
          const errorMessage = ctx.error.message || 'Failed to send verification email.'
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      ...
      <div className="space-y-2.5">
        <Label className="text-sm font-semibold tracking-tight" htmlFor={emailInputId}>
          Email Address
        </Label>
      ...
      <p className="text-xs text-muted-foreground text-center pt-2 leading-relaxed">
        Check your spam or promotions folder if you don't see the email within a few minutes.
      </p>
    </form>
  )
}
