...
function RouteComponent() {
  ...
  const play = async () => {
    if (betAmount < 1 || betAmount > balance || selectedNumbers.size === 0) return
    ...
    // Calculate matches
    const matchCount = Array.from(selectedNumbers).filter((num) => drawn.has(num)).length
      ...
      <Card className="border-border/50 bg-card/50 p-8 backdrop-blur-sm">
              ...
              return (
                <button
                  className={cn(
                    ...
                    isSelected && !isDrawn && 'border-primary bg-primary/20 text-primary',
                    isDrawn && !isSelected && 'border-muted bg-muted/50 text-muted-foreground',
                    isMatch &&
        ...
        <div className="mb-6 grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <p className="text-xl font-bold text-primary">{selectedNumbers.size}/8</p>
          </div>
          <div>
        ...
        {payout > 0 && !isPlaying && (
          <Card className="mb-6 border-primary/50 bg-primary/10 p-4">
            <div className="text-center">
              <p className="text-lg font-semibold text-primary">Won ${payout.toFixed(2)}!</p>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button
              ...
              disabled={
                isPlaying || betAmount < 1 || betAmount > balance || selectedNumbers.size === 0
              }
              onClick={play}
              size="lg"
            >
            ...
            <Button disabled={isPlaying} onClick={clearSelection} size="lg" variant="outline">
              Clear
            </Button>
          </div>
        </div>
