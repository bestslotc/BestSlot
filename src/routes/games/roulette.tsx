...
function RouteComponent() {
  ...
  const _blackNumbers = [2, 4, 6, 8, 10, 11, 13, 15, 17, 20, 22, 24, 26, 28, 29, 31, 33, 35]
  ...
  const calculateWinnings = (num: number) => {
    ...
    if (totalWin > 0) {
      ...
      setMessage(`Lost $${bets.reduce((sum, bet) => sum + bet.amount, 0).toFixed(2)}`)
    }
      ...
      <Card className="border-border/50 bg-card/50 p-8 backdrop-blur-sm">
        ...
        <div className="mb-6 flex flex-col items-center justify-center gap-4 rounded-xl bg-accent/30 p-8">
          <div
            className={cn(
              ...
              result !== null && getNumberColor(result) === 'red' && 'border-red-500 bg-red-500/20',
              result !== null &&
          ...
          >
            <div className="text-5xl font-bold text-primary">{result !== null ? result : '?'}</div>
          </div>
          {result !== null && !isSpinning && (
            <div className="text-center">
              <p className="text-sm text-muted-foreground capitalize">{getNumberColor(result)}</p>
            </div>
          )}
        </div>
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="rounded-lg border border-border/50 bg-accent/30 p-4">
            <p className="mb-2 text-xs text-muted-foreground">Straight Bets (35:1)</p>
                ...
                return (
                  <button
                    className={cn(
                      ...
                      color === 'black' && 'bg-foreground text-background hover:bg-foreground/90'
                    )}
                    disabled={isSpinning || totalBetAmount + betAmount > balance}
                    key={num}
        ...
        {bets.length > 0 && (
          <Card className="mb-4 border-primary/50 bg-primary/10 p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Active Bets: {bets.length}</p>
                <p className="font-semibold text-primary">Total: ${totalBetAmount.toFixed(2)}</p>
              </div>
              <Button disabled={isSpinning} onClick={clearBets} size="sm" variant="outline">
                Clear Bets
              </Button>
            </div>
          </Card>
