...
function RouteComponent() {
  ...
  const { title, description } = errorMessages[errorKey] || errorMessages.invalid_credentials
      ...
      <div className="w-full max-w-md">
        <div className="bg-card border border-border rounded-2xl shadow-sm p-8 space-y-6 animate-in fade-in zoom-in duration-500">
          ...
          <div className="space-y-3 pt-2">
            <Link className="block w-full" to="/auth/signin">
          ...
          <div className="border-t border-border pt-4 text-center">
            <p className="text-xs text-muted-foreground">
              Need help?{' '}
              <Link className="text-primary hover:underline font-medium" to="/support">
                Contact support
              </Link>
            </p>
          </div>
